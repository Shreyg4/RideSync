...
const signUp = () => {
  ...
  type FieldErrors = Partial<Record<'firstName' | 'lastName' | 'username' | 'email' | 'password' | 'confirmPassword', string>>
  ...
  useEffect(() => {
    ...
    if (candidate.length < 5 || !HANDLE.test(candidate)) { setUsernameState('idle'); return }
  ...
  const handleSignUp=async() => {
    ...
    try{
      ...
      const { data: stillFree } = await supabase.rpc('username_available', { candidate: clean.username })
  ...
  return (
    <View style={{flex: 1}}>
      ...
      <KeyboardAwareScrollView showsVerticalScrollIndicator={false} 
        ...
        <TextBox value={lastName} onChangeText={updateField('lastName', setLastName)} error={!!errors.lastName} placeholder='Last Name'/>
        ...
        <TextBox value={username} onChangeText={updateField('username', setUsername)} error={!!errors.username} placeholder='Username' autoCapitalize='none' maxLength={20}/>
        {errors.username ? <Text style={styles.errorText}>{errors.username}</Text> : <Text style={styles.infoText}>5–20 characters, letters, numbers and underscores</Text>}
        {availability ? (
          <View style={styles.availabilityRow}>
            <Text style={[styles.availablityText, { color: availability.color, marginLeft: 0 }]}>{availability.text}</Text>
        ...
        {errors.password ? <Text style={styles.errorText}>{errors.password}</Text> : <Text style={styles.infoText}>Must be at least 8-characters long</Text>}

        <TextBox value={confirmPassword} onChangeText={updateField('confirmPassword', setConfirmPassword)} error={!!errors.confirmPassword} placeholder='Re-enter password' secureTextEntry={true}/>
